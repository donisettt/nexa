...
export default function LoginForm({ onAuthenticated, onNavigate }) {
    ...
    const submit = async (event) => {
        ...
        try {
            const { data } = await window.axios.post('/auth/login', form);
    ...
    return (
        <form className="w-full space-y-5" onSubmit={submit}>
            ...
            <div className="flex items-center justify-between text-sm">
                <label className="flex items-center gap-2 text-[#5a4d6e] cursor-pointer">
                    <input
                        checked={form.remember}
                        className="h-4 w-4 rounded border-gray-300 text-[#6366f2] focus:ring-[#6366f2]"
                        name="remember"
                ...
                <button type="button" className="text-xs font-semibold text-[#6366f2] hover:underline focus:outline-none">
                    Forgot Password?
                </button>
            </div>
            ...
            {/* Submit Button */}
            <button
                className="flex w-full items-center justify-center rounded-lg bg-[#6366f2] px-4 py-3 text-sm font-bold text-white shadow-sm transition hover:bg-[#4f46e5] focus:outline-none focus:ring-4 focus:ring-[#6366f2]/20 disabled:cursor-not-allowed disabled:opacity-70"
                disabled={submitting}
                type="submit"
            >
